...
def evaluate_model(
    y_true, y_pred, y_proba=None, labels=None, target_names=None
) -> Dict:
    """
...
def plot_comparison_quadrant(results_df: pd.DataFrame, save_path: Optional[str] = None):
    """
    ...
    ax.set_title(
        "🎯 Quadrant Plot: Performance vs. Efficiency", fontsize=14, fontweight="bold"
    )
...
def error_analysis(
    y_true, y_pred, texts, target_names=None, n_examples: int = 10
) -> Dict:
    """
...
def generate_comparison_table(
    all_results: List[Dict], save_path: str = "results/comparison_table.csv"
) -> pd.DataFrame:
    """
...
def print_top_results(comparison_df: pd.DataFrame, n: int = 10):
    ...
    print("=" * 90)
    print(
        f"{'Rank':>4} | {'Feature Extractor':>18} | {'Model':>22} | "
    ...
    for idx, row in comparison_df.head(n).iterrows():
        ...
        print(
            f"{idx:>4} | {row['feature_extractor']:>18} | {row['model']:>22} | "
...
def plot_rating_distribution(df: pd.DataFrame, save_path: Optional[str] = None):
    ...
    ax.set_title("📊 Distribusi Rating Ulasan CoreTax", fontsize=14, fontweight="bold")
...
def plot_monthly_trend(df: pd.DataFrame, save_path: Optional[str] = None):
    ...
    ax.fill_between(monthly["year_month"], monthly["count"], alpha=0.1, color="#3498DB")
...
def plot_wordcloud(texts, title: str = "Word Cloud", save_path: Optional[str] = None):
